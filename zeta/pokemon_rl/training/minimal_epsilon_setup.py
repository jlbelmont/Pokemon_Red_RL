...
from ..envs.env_pokemon import PokemonRedEnv
...
from ..agents.simple_dqn import SimpleDQN, LowLevelDQNPolicy
...
from ..bayesian_proj.progress_tracking import (
    BADGE_NAMES,
...
def _format_event_counts(counter: Counter[str], max_items: int = 6, pair_sep: str = ";") -> str:
    if not counter:
        return ""
    items = sorted(counter.items(), key=lambda kv: (-kv[1], str(kv[0]).lower()))
    selected = items[:max_items]
    return pair_sep.join(f"{name}:{count}" for name, count in selected)


def _coverage_stats(
    unique_tiles: list[int],
    total_tiles: list[int],
    new_towns: list[set[str]],
) -> dict:
    if not unique_tiles:
        return {
            "mean_unique_tiles": 0.0,
            "max_unique_tiles": 0,
            "max_total_tiles": 0,
            "new_towns": [],
        }
    towns_union: set[str] = set()
    for entries in new_towns:
        for town in entries:
            if town:
                towns_union.add(str(town))
    return {
        "mean_unique_tiles": float(np.mean(unique_tiles)) if unique_tiles else 0.0,
...
def _format_coverage_tiles(stats: dict) -> str:
    if not stats:
        return ""
    return (
...
def _format_coverage_towns(stats: dict) -> str:
    if not stats:
        return ""
...
def _format_coverage_console(stats: dict) -> str:
    if not stats:
        return "tiles μ0|max0 world0 | towns+0"
    towns = stats.get("new_towns") or []
    return (
...
class VideoRecorder:
    """Stream frames to a compressed video file using imageio."""

    def __init__(self, path: str, fps: float = 30.0, duration_seconds: float = 300.0) -> None:
        self.path = path
        self.fps = max(1, int(round(fps)))
        self.duration_seconds = max(1.0, float(duration_seconds))
        self.max_frames = max(1, int(self.fps * self.duration_seconds))
        self.frames_written = 0
        self._done = False
        self._writer = None
        self.available = imageio is not None
        if not self.available:
            print("[video] imageio is missing; skipping video export.")
            return
        directory = os.path.dirname(self.path)
        if directory:
            os.makedirs(directory, exist_ok=True)
        try:
            ...
            print(f"[video] Unable to open video writer ({self.path}): {exc}")
    ...
    def add_frame(self, frame: Optional[np.ndarray]) -> None:
        if not self.available or self._writer is None or frame is None or self._done:
            return
        array = np.asarray(frame)
        ...
        try:
            ...
            print(f"[video] Failed to append frame: {exc}")
            self.close()
            return
...
class TrainConfig:
    ...
    aggregate_layout_path: str | None
    aggregate_background_path: str | None
    aggregate_tile_scale: int
    aggregate_pad_tiles: int
    aggregate_overlay_alpha: float
    vectorized: bool
    ...
    post_training_video_enabled: bool
    post_training_video_path: str | None
    post_training_video_seconds: float
    post_training_video_fps: float
    post_training_video_checkpoint: str | None
...
class CurriculumManager:
    ...
    def load_state_dict(self, state: dict | None) -> None:
        if not state:
            return
        saved = state.get("indices") or []
        if not saved:
            return
        for idx, value in enumerate(saved):
            self.indices[idx % len(self.indices)] = int(value) % max(1, len(self.sequence) or 1)
...
class ReplayBuffer:
    ...
    def state_dict(self, limit: int | None = None) -> dict:
        count = len(self.storage)
        if limit is not None and limit > 0:
            count = min(count, limit)
        snapshot = self.storage[-count:] if count else []
        return {
    ...
    def load_state_dict(self, state: dict) -> None:
        ...
        stored = state.get("storage") or []
        self.storage = list(stored)[-self.capacity :]
        self.position = int(state.get("position", len(self.storage))) % max(1, self.capacity)
        priorities = state.get("priorities")
        if isinstance(priorities, np.ndarray):
            arr = np.zeros(self.capacity, dtype=np.float32)
            length = min(priorities.size, self.capacity)
            arr[:length] = priorities[:length]
            self.priorities = arr
        elif priorities:
            temp = np.asarray(priorities, dtype=np.float32)
            arr = np.zeros(self.capacity, dtype=np.float32)
            length = min(temp.size, self.capacity)
    ...
    def save_to_path(self, path: str, limit: int | None = None) -> dict | None:
        state = self.state_dict(limit)
        if not state.get("storage"):
            return None
        tmp_path = f"{path}.tmp"
        torch.save(state, tmp_path)
        os.replace(tmp_path, path)
        return {"path": path, "count": len(state["storage"])}
    ...
    def load_from_path(self, path: str) -> None:
        ...
        try:
            ...
            print(f"[replay] Failed to load replay buffer from {path}: {exc}")
            return
    ...
    for penalty_cfg in getattr(cfg, "map_stay_penalties", []) or []:
        modules.append(
            MapStayPenalty(
                ...
                map_names=penalty_cfg.get("map_names"),
...
def parse_args():
    ...
    default_rom = os.path.join(os.path.dirname(os.path.dirname(__file__)), "pokemon_red.gb")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
        help="Path to JSON config file (defaults to configs/training_config.json).",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="Path to a training checkpoint to resume from (defaults to dqn_route1_latest.pt when present).",
    )
    parser.add_argument(
        "--fresh-start",
        action="store_true",
    ...
    parser.add_argument(
        "--replay-buffer-path",
        default="checkpoints/replay_buffer.pt",
        help="File used to persist replay buffer snapshots between runs.",
    )
    parser.add_argument(
        "--replay-buffer-max-save",
        type=int,
        default=250000,
        help="Maximum transitions saved when checkpointing the replay buffer (0 disables persistence).",
    )
    parser.add_argument(
        "--log-interval",
        type=int,
    ...
    parser.add_argument(
        "--aggregate-layout-path",
        default=None,
        help="JSON layout describing how each map id projects into the stitched Kanto background.",
    )
    parser.add_argument(
        "--aggregate-background-path",
        default=None,
        help="Optional PNG background used behind the aggregate heatmap.",
    )
    parser.add_argument(
        "--aggregate-tile-scale",
        type=int,
        default=16,
        help="Pixel size of each tile when rendering the aggregate heatmap.",
    )
    parser.add_argument(
        "--aggregate-pad-tiles",
        type=int,
        default=12,
        help="Padding (in tiles) added around the aggregate layout to center the map.",
    )
    parser.add_argument(
        "--aggregate-overlay-alpha",
        type=float,
        default=0.72,
        help="Alpha multiplier used when compositing the heatmap over the background image.",
    )
    parser.add_argument(
        "--show-env-maps",
        action="store_true",
    ...
    parser.add_argument(
        "--post-training-video",
        dest="post_training_video_enabled",
        action="store_true",
        help="Record a replay video after training completes.",
    )
    parser.add_argument(
        "--no-post-training-video",
        dest="post_training_video_enabled",
        action="store_false",
        help="Disable post-training replay video recording.",
    )
    parser.set_defaults(post_training_video_enabled=None)
    parser.add_argument(
        "--post-training-video-path",
        default=None,
        help="Custom output path for the post-training replay video.",
    )
    parser.add_argument(
        "--post-training-video-seconds",
        type=float,
        default=None,
        help="Duration (seconds) for the post-training replay video.",
    )
    parser.add_argument(
        "--post-training-video-fps",
        type=float,
        default=None,
        help="Frames per second for the post-training replay video.",
    )
    parser.add_argument(
        "--post-training-video-checkpoint",
        default=None,
        help="Checkpoint to load when recording the post-training replay video.",
    )
    parser.add_argument(
        "--replay-checkpoint",
        default=None,
    ...
    parser.add_argument(
        "--replay-video-output",
        default=None,
        help="Optional MP4 path for saving the replay session while using --watch-only.",
    )
    parser.add_argument(
        "--replay-video-seconds",
        type=float,
        default=0.0,
        help="Duration (seconds) for replay videos recorded via --replay-video-output (0 uses config default).",
    )
    parser.add_argument(
        "--replay-video-fps",
        type=float,
        default=30.0,
        help="Frames per second for replay videos recorded via --replay-video-output.",
    )
    parser.add_argument(
        "--demo-episodes",
        type=int,
    ...
    additional_defaults = {
        ...
        "post_training_video_path": "checkpoints/best_playthrough.mp4",
        "post_training_video_seconds": 300.0,
        "post_training_video_fps": 30.0,
        "post_training_video_checkpoint": None,
...
def build_config(args) -> TrainConfig:
    ...
    if aggregate_layout_path:
        if config_dir and not os.path.isabs(aggregate_layout_path):
            aggregate_layout_path = os.path.normpath(os.path.join(config_dir, aggregate_layout_path))
        else:
            aggregate_layout_path = os.path.normpath(aggregate_layout_path)
        setattr(args, "aggregate_layout_path", aggregate_layout_path)
    aggregate_background_path = getattr(args, "aggregate_background_path", None)
    if aggregate_background_path:
        if config_dir and not os.path.isabs(aggregate_background_path):
            aggregate_background_path = os.path.normpath(os.path.join(config_dir, aggregate_background_path))
        else:
            aggregate_background_path = os.path.normpath(aggregate_background_path)
    ...
    if isinstance(resume_checkpoint, str) and resume_checkpoint.strip():
        resume_checkpoint = os.path.normpath(resume_checkpoint.strip())
    else:
        resume_checkpoint = None
    fresh_start = bool(getattr(args, "fresh_start", False))
    replay_buffer_path = getattr(args, "replay_buffer_path", None)
    replay_buffer_path = _resolve_path_like(replay_buffer_path, getattr(args, "save_dir", "")) if replay_buffer_path else None
    replay_buffer_max_save = max(0, int(getattr(args, "replay_buffer_max_save", 0)))
    ...
    return TrainConfig(
        ...
        aggregate_layout_path=getattr(args, "aggregate_layout_path", None),
        aggregate_background_path=getattr(args, "aggregate_background_path", None),
        aggregate_tile_scale=max(1, int(getattr(args, "aggregate_tile_scale", 16))),
        aggregate_pad_tiles=max(0, int(getattr(args, "aggregate_pad_tiles", 12))),
        aggregate_overlay_alpha=float(getattr(args, "aggregate_overlay_alpha", 0.72)),
        ...
        replay_buffer_max_save=replay_buffer_max_save,
        ...
        post_training_video_enabled=bool(getattr(args, "post_training_video_enabled", True)),
        post_training_video_path=getattr(args, "post_training_video_path", None),
        post_training_video_seconds=float(
            getattr(args, "post_training_video_seconds", 300.0)
        ),
        post_training_video_fps=float(getattr(args, "post_training_video_fps", 30.0)),
        post_training_video_checkpoint=getattr(args, "post_training_video_checkpoint", None),
...
def _best_checkpoint_path(cfg: TrainConfig) -> str:
    return os.path.join(cfg.save_dir, "dqn_route1_best.pt")
...
def _resolve_checkpoint_path(cfg: TrainConfig, explicit: str | None) -> str | None:
    ...
    candidates.append(_best_checkpoint_path(cfg))
...
def _resolve_resume_checkpoint(cfg: TrainConfig, explicit: str | None) -> str | None:
    candidates: list[str] = []
    if explicit:
        candidate = explicit
        if not os.path.isabs(candidate):
            candidate = os.path.normpath(os.path.join(cfg.save_dir, candidate))
    ...
    try:
        ckpt_dir = Path(cfg.save_dir)
        if ckpt_dir.is_dir():
            for pt in sorted(ckpt_dir.glob("*.pt"), key=lambda p: p.stat().st_mtime, reverse=True):
                candidates.append(str(pt))
    except Exception:
        pass
    for candidate in candidates:
...
def _maybe_record_post_training_video(cfg: TrainConfig) -> None:
    if not cfg.post_training_video_enabled:
        return
    explicit = cfg.post_training_video_checkpoint
    checkpoint = _resolve_checkpoint_path(cfg, explicit) if explicit else _resolve_checkpoint_path(cfg, None)
    if not checkpoint:
    ...
    recorder = VideoRecorder(
        ...
        duration_seconds=cfg.post_training_video_seconds,
    )
    if not recorder.available:
        return
    print(f"[video] Recording replay video to {output_path} ...")
    video_cfg = replace(
        cfg,
        num_envs=1,
        render_map=False,
        show_env_maps=False,
        render_gameplay_grid=False,
        display_envs=0,
    )
    try:
        run_replay(
            ...
            checkpoint,
            episodes=1,
            max_steps=int(cfg.max_steps_per_episode * 2),
            save_map_path=None,
            show_progress=False,
            video_recorder=recorder,
        )
    except Exception as exc:
        print(f"[video] Failed to record replay video: {exc}")
...
def save_training_state(
    ...
    rnd_normalizer,
    curriculum_manager,
    dynamic_rnd_scale: float,
    best_reward: float,
    global_step: int,
    episode: int,
    reward_window: deque,
    unlocked_badges: set[str],
    unlocked_story_flags: set[str],
    ...
    state: dict[str, object] = {
        ...
        "dynamic_rnd_scale": dynamic_rnd_scale,
        "unlocked_badges": sorted(unlocked_badges),
        "unlocked_story_flags": sorted(unlocked_story_flags),
    }
    if visit_counter:
        state["visit_counter"] = visit_counter.state_dict()
    if episodic_memory:
        state["episodic_memory"] = episodic_memory.state_dict()
    if rnd_module:
        state["rnd_module"] = rnd_module.state_dict()
    if rnd_optimizer:
        state["rnd_optimizer"] = rnd_optimizer.state_dict()
    if rnd_normalizer:
        state["rnd_normalizer"] = rnd_normalizer.state_dict()
    if curriculum_manager:
        state["curriculum_state"] = curriculum_manager.state_dict()
    rng_state = {
    ...
    if (
        replay_buffer
        and replay_snapshot_path
        and cfg.replay_buffer_max_save > 0
    ):
        try:
            buffer_meta = replay_buffer.save_to_path(
                replay_snapshot_path, cfg.replay_buffer_max_save
            )
        except Exception as exc:
            print(f"[checkpoint] Warning: failed to snapshot replay buffer: {exc}")
    if buffer_meta:
        state["replay_buffer_meta"] = buffer_meta
    tmp_path = f"{path}.tmp"
    torch.save(state, tmp_path)
    os.replace(tmp_path, path)
...
def load_training_state(
    ...
    try:
        ...
        print(f"[checkpoint] Failed to load training state {path}: {exc}")
        return None
    if not isinstance(state, dict) or "model" not in state:
        try:
            online_net.load_state_dict(state)
            target_net.load_state_dict(state)
        except Exception as exc:
            print(f"[checkpoint] Legacy checkpoint {path} could not be loaded: {exc}")
            return None
        print(f"[checkpoint] Loaded legacy checkpoint {path}; optimizer state unavailable.")
        return None
    online_net.load_state_dict(state["model"])
    target_state = state.get("target_model") or state["model"]
    ...
    if opt_state:
        try:
            ...
            print(f"[checkpoint] Warning: failed to load optimizer state: {exc}")
    if rnd_module and state.get("rnd_module"):
        try:
            rnd_module.load_state_dict(state["rnd_module"])
        except Exception as exc:
            print(f"[checkpoint] Warning: failed to load RND module: {exc}")
    if rnd_optimizer and state.get("rnd_optimizer"):
        try:
            rnd_optimizer.load_state_dict(state["rnd_optimizer"])
        except Exception as exc:
            print(f"[checkpoint] Warning: failed to load RND optimizer: {exc}")
    if rnd_normalizer and state.get("rnd_normalizer"):
        rnd_normalizer.load_state_dict(state["rnd_normalizer"])
    if visit_counter and state.get("visit_counter"):
        visit_counter.load_state_dict(state["visit_counter"])
    if episodic_memory and state.get("episodic_memory"):
        episodic_memory.load_state_dict(state["episodic_memory"])
    if curriculum_manager and state.get("curriculum_state"):
        curriculum_manager.load_state_dict(state["curriculum_state"])
    replay_meta = state.get("replay_buffer_meta")
    if replay_meta and replay_buffer:
        buffer_path = replay_meta.get("path")
        try:
            replay_buffer.load_from_path(buffer_path)
        except Exception as exc:
            print(f"[checkpoint] Warning: failed to load replay buffer from {buffer_path}: {exc}")
    ...
    return {
        ...
        "reward_window": state.get("reward_window", []),
        "dynamic_rnd_scale": float(state.get("dynamic_rnd_scale", 0.0)),
        "unlocked_badges": state.get("unlocked_badges", []),
        "unlocked_story_flags": state.get("unlocked_story_flags", []),
    }
...
def run_replay(
    ...
    if not checkpoint:
        ...
        return
    if video_recorder and not video_recorder.available:
        video_recorder = None
    ...
    if render_map and cfg.aggregate_layout_path:
        try:
            aggregate_projector = KantoAggregateProjector(
                cfg.aggregate_layout_path,
                background_path=cfg.aggregate_background_path,
                tile_scale=cfg.aggregate_tile_scale,
                pad_tiles=cfg.aggregate_pad_tiles,
                overlay_alpha=cfg.aggregate_overlay_alpha,
            )
        except Exception as exc:
            print(f"[replay] Aggregate map overlay disabled: {exc}")
            aggregate_projector = None
    ...
    try:
        for episode in range(episodes):
            ...
            for idx, env in enumerate(envs):
                ...
                if video_recorder and idx == 0:
                    frame_for_video = raw_frame if isinstance(raw_frame, np.ndarray) else obs
        ...
        if video_complete:
            print(
                f"[video] Recorded {video_recorder.frames_written if video_recorder else 0} frames to {video_recorder.path if video_recorder else 'N/A'}"
            )
    finally:
        if video_recorder:
...
def train(cfg: TrainConfig) -> None:
    ...
    gameplay_viz = None
    aggregate_projector = None
    if cfg.render_map and cfg.aggregate_layout_path:
        try:
            aggregate_projector = KantoAggregateProjector(
                cfg.aggregate_layout_path,
                background_path=cfg.aggregate_background_path,
    ...
    replay_snapshot_path = cfg.replay_buffer_path
    start_episode = 0
    resume_meta: dict | None = None
    resume_state_path = None
    if not cfg.fresh_start:
        resume_state_path = _resolve_resume_checkpoint(cfg, cfg.resume_checkpoint_path)
        if resume_state_path:
            print(f"[checkpoint] Resuming training from {resume_state_path}")
            resume_meta = load_training_state(
                resume_state_path,
                online_net,
                target_net,
                optimizer,
            ...
            if resume_meta is None:
                print("[checkpoint] Resume checkpoint did not contain training state; starting fresh.")
    global_step = 0
    reward_window = deque(maxlen=25)
    best_reward = -float("inf")
    if resume_meta:
        start_episode = int(resume_meta.get("episode", 0))
        global_step = int(resume_meta.get("global_step", 0))
        best_reward = float(resume_meta.get("best_reward", best_reward))
        history = resume_meta.get("reward_window") or []
        reward_window.extend(history)
        dynamic_rnd_scale = float(resume_meta.get("dynamic_rnd_scale", dynamic_rnd_scale))
        unlocked_badges_global.update(resume_meta.get("unlocked_badges") or [])
        unlocked_story_flags_global.update(resume_meta.get("unlocked_story_flags") or [])
    else:
        start_episode = 0
    last_loss = None
    best_model_path = _best_checkpoint_path(cfg)
    ...
    last_episode_index = start_episode
    ...
    try:
        ...
        for episode in range(start_episode, cfg.episodes):
            ...
            episode_event_results: list[dict[str, bool]] = []
            coverage_unique_tiles = [0] * num_envs
            coverage_total_tiles = [0] * num_envs
            episode_new_towns = [set() for _ in range(num_envs)]
            for idx, env in enumerate(envs):
                seed = cfg.seed + episode * num_envs + idx
                base_state = cfg.state_path
                state_label = None
            ...
            for step in range(cfg.max_steps_per_episode):
                ...
                for idx, env in enumerate(envs):
                    ...
                    if intrinsic_total_map:
                        ...
                        for comp_name, comp_value in intrinsic_total_map.items():
                            try:
                                ...
                                continue
                    unique_tiles_val = int(next_info.get("episode_unique_tiles") or 0)
                    if unique_tiles_val > coverage_unique_tiles[idx]:
                        coverage_unique_tiles[idx] = unique_tiles_val
                    total_tiles_val = int(next_info.get("total_locations_visited") or 0)
                    if total_tiles_val > coverage_total_tiles[idx]:
                        coverage_total_tiles[idx] = total_tiles_val
                    town_name = next_info.get("new_town_visited")
                    if town_name:
                        episode_new_towns[idx].add(str(town_name))
                    ...
                    if done_flag and progress_tracker:
                        ...
                        if result:
                            episode_event_results.append(result)
                ...
                if all(done) or not active or video_complete:
                    break
            for idx in range(num_envs):
                flush_nstep(idx, force=True)
            if progress_tracker:
            ...
            quest_counts = _summarize_event_hits(episode_event_results)
            coverage_stats = _coverage_stats(
                coverage_unique_tiles,
                coverage_total_tiles,
                episode_new_towns,
            )
            ...
            quest_console = _format_event_counts(quest_counts, pair_sep=", ") or "none"
            coverage_console = _format_coverage_console(coverage_stats)
            if summary_due:
                print(
                    f"Episode {episode + 1:04d} | Rewards [{rewards_str}] | Mean {mean_reward:8.2f} | "
                    f"Avg(25) {running_avg:8.2f} | Steps {global_step:7d} | "
                    f"Epsilon {epsilon:6.3f} | Loss {loss_val:8.4f} | Quest {quest_console} | "
                    f"Coverage {coverage_console}",
                    flush=True,
                )
            ...
            if now - last_auto_save >= auto_save_seconds:
                ...
                last_auto_save = now
                print(
                    f"[checkpoint] Autosaved latest training state at episode {episode + 1:04d}"
                )
...
if __name__ == "__main__":
    ...
    if arguments.watch_only:
        replay_video = None
        if arguments.replay_video_output:
            output_path = arguments.replay_video_output
            if not os.path.isabs(output_path):
                output_path = os.path.join(config.save_dir, output_path)
            duration = arguments.replay_video_seconds or config.post_training_video_seconds
            fps = arguments.replay_video_fps or config.post_training_video_fps
            replay_video = VideoRecorder(output_path, fps=fps, duration_seconds=duration)
            if not replay_video.available:
                replay_video = None
        run_replay(
            config,
            ...
            video_recorder=replay_video,
        )
    else:
        train(config)
        _maybe_record_post_training_video(config)
